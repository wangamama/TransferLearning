...
def tradaboost(trans_S, trans_A, label_S, label_A, test,test_label, N,eliminate=True):
    ...
    row_T = test.shape[0]
    # 样本重标记
    lable_spy_a = np.zeros([row_A, 1])
    lable_spy_s = np.ones([row_S, 1])

    trans_data = sparse.vstack((trans_A, trans_S))
    trans_label=np.concatenate((lable_spy_a, lable_spy_s), axis=0)
    ...
    #排除一些低权重的样本
    # 排除一些低权重的样本
    # print(clf.predict_proba(trans_A)[:,-1])


    weights_A = clf.predict_proba(trans_A)[:,-1].tolist()

    if eliminate:
        percenttile=np.percentile(weights_A, 32)#32
        indexlist=[]
        for index,x in enumerate(weights_A):
            if(x<percenttile):
    ...
    for i in range(N):
        ...
        #     left = np.sum(
        #         result_label[row_A + row_S + i, 0:iteration] * np.log(1 / bata_T[0, 0:iteration]))
